...
def register_callbacks(app):
    ...
    # === SHOW/HIDE POLYA SECTION =============================
    @app.callback(
        Output('polya-section', 'style'),
        Input('selected-mode', 'children')
    )
    def toggle_polya_section_visibility(mode):
        if mode == 'Intron':
    ...
    def update_gene_options(search_value, mode, species, cell_type, tag_length, current_value, recent_genes_json):
        ...
        if not gene_ids:
            ...
            if not os.path.exists(db_path):
                return [], 'Guides for this gene have not been pre-calculated.', None, '' if trigger_id == 'species-dropdown' else dash.no_update
    ...
    def clear_visualization_on_db_change(mode, cell_type, species, tag_length, selected_gene):
        ...
        if not os.path.exists(db_path):
            message_div = html.Div(
                "Guides for this gene have not been pre-calculated.",
                style={
                    'textAlign': 'center',
    ...
    def display_gene_table(selected_assembly, mode, tag_length, species, cell_type, selected_gene):
        ...
        try:
            ...
            with get_db_connection(db_path) as conn:
                ...
                if is_mouse:
                    query = f"""
                        SELECT DISTINCT m.gRNA, m.CRISPRScan_score, m.integration_score, m.location, m.strand,
    ...
    def update_mutation_detail(
        ...
        last_preset_components = None

        if custom_data and custom_data.get('preset_info'):
            _all_items = (TAG_PRESETS.get('linkers', []) +
                          TAG_PRESETS.get('tags', []) +
                          TAG_PRESETS.get('polya', []))
            _type_map = {
                **{l['name']: 'Linker' for l in TAG_PRESETS.get('linkers', [])},
                **{t['name']: 'Tag'    for t in TAG_PRESETS.get('tags', [])},
                **{p['name']: 'PolyA'  for p in TAG_PRESETS.get('polya', [])},
            }
            last_preset_components = []
            for name in custom_data['preset_info'].get('all_components', []):
                for item in _all_items:
                    if item['name'] == name:
                        last_preset_components.append({
                            'name': name,
                            'type': _type_map.get(name, 'Component'),
                            'color': item.get('color', '#6B7280'),
                            'length': len(item.get('sequence', '') or '')
                        })
                        break

        custom_seq_panel = build_custom_sequence_panel(
            current_sequence=current_seq,
            last_preset_components=last_preset_components
        )
    ...
    @app.callback(
        [
            ...
            Output('polya-warning', 'children'),
        ],
        [
            Input('linker-dropdown', 'value'),
            Input('tag-dropdown', 'value'),
            Input('polya-dropdown', 'value'),
    ...
    def display_preset_sequence_info(linker_name, tag_name, polya_name, additional_values, additional_types):
        polya_warning = html.P(
            "No polyA selected — transcript termination may be unreliable",
            style={'fontSize': '0.875rem', 'color': '#D97706', 'fontWeight': '600',
                   'fontStyle': 'italic', 'padding': '0.5rem 0.75rem',
                   'backgroundColor': '#FEF3C7', 'borderRadius': '0.5rem',
                   'border': '1px solid #F59E0B', 'margin': '0'}
        ) if polya_name == 'none' else ""

        if not linker_name or not tag_name:
            return "", polya_warning

        linker_seq, tag_seq = None, None
        linker_color, tag_color = "#8B4513", "#10B981"
        ...
        components = [
            {'name': linker_name, 'sequence': linker_seq, 'color': linker_color, 'type': 'Linker'},
            {'name': tag_name,    'sequence': tag_seq,    'color': tag_color,    'type': 'Tag'},
        ]
                ...
                for item in search_list:
                    if item['name'] == value:
                        components.append({
                            ...
                            'color': item.get('color', "#8B4513" if comp_type == "linker" else "#10B981"),
                            'type': comp_type
                        })
                        break

        if polya_name and polya_name != 'none':
            for p in TAG_PRESETS.get('polya', []):
                if p['name'] == polya_name:
                    components.append({
                        'name': p['name'],
                        'sequence': p['sequence'],
        ...
        combined_length = sum(len(c['sequence']) for c in components)

        badges = [
            html.Span(
                f"{c['type']}: {len(c['sequence'])} bp",
                style={
        ...
        return html.Div([
            html.Div("Selected components",
                     style={'fontWeight': '600', 'fontSize': '0.8rem', 'color': '#6B7280', 'marginBottom': '6px'}),
            html.Div(badges, style={'display': 'flex', 'flexWrap': 'wrap', 'marginBottom': '6px'}),
            html.Div(f"{combined_length} bp total",
                     style={'fontSize': '0.8rem', 'color': '#6B7280', 'fontStyle': 'italic'})
        ], style={
            'padding': '12px', 'backgroundColor': '#F3F4F6',
    ...
    def handle_preset_recalculation(
        n_clicks, linker_name, tag_name, polya_name, additional_values, additional_types,
        ...
        # Append polyA DNA sequence to the cassette
        if polya_name and polya_name != 'none':
            for p in TAG_PRESETS.get('polya', []):
                if p['name'] == polya_name:
                    combined_sequence += p['sequence']
                    component_names.append(polya_name)
                    break
